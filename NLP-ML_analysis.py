...
def evaluate_regression_model(model, X_train, y_train, X_test, y_test, model_name):
    ...
    # Converting regression predictions to binary for classification evaluation
...
#  Linear Regression 
evaluate_regression_model(LinearRegression(), X_train, y_train, X_test, y_test, "Linear Regression")

# Ridge Regression
evaluate_regression_model(Ridge(alpha=1.0), X_train, y_train, X_test, y_test, "Ridge Regression")

# Lasso Regression 
evaluate_regression_model(Lasso(alpha=0.01, max_iter=1000), X_train, y_train, X_test, y_test, "Lasso Regression")

# ElasticNet combines L1 (Lasso) and L2 (Ridge) regularization
evaluate_regression_model(ElasticNet(alpha=0.1, l1_ratio=0.5, max_iter=1000), X_train, y_train, X_test, y_test, "ElasticNet Regression")

# SGDRegressor (Stochastic Gradient Descent Regressor)
evaluate_regression_model(SGDRegressor(max_iter=1000, tol=1e-3, random_state=0), X_train, y_train, X_test, y_test, "SGD Regressor")

# HuberRegressor
evaluate_regression_model(HuberRegressor(max_iter=1000), X_train, y_train, X_test, y_test, "Huber Regressor")

#  Random Forest Regressor
evaluate_regression_model(RandomForestRegressor(n_estimators=100, random_state=0), X_train, y_train, X_test, y_test, "Random Forest Regressor")

#  Support Vector Regressor (SVR)
evaluate_regression_model(SVR(kernel='rbf'), X_train, y_train, X_test, y_test, "Support Vector Regressor (SVR)")
...
try:
    evaluate_regression_model(poly_pipeline, X_train, y_train, X_test, y_test, "Polynomial Regression (Degree 2)")
...
evaluate_regression_model(MLPRegressor(hidden_layer_sizes=(100,), max_iter=500, random_state=0, solver='adam'), X_train, y_train, X_test, y_test, "MLP Regressor")

#  K-Nearest Neighbors Regressor
evaluate_regression_model(KNeighborsRegressor(n_neighbors=5), X_train, y_train, X_test, y_test, "K-Neighbors Regressor")

#  LightGBM Regressor
evaluate_regression_model(lgb.LGBMRegressor(random_state=0), X_train, y_train, X_test, y_test, "LightGBM Regressor")

#  XGBoost Regressor
evaluate_regression_model(xgb.XGBRegressor(objective='reg:squarederror', n_estimators=100, random_state=0), X_train, y_train, X_test, y_test, "XGBoost Regressor")
